...
export default function ExplainStudentsMiddle() {
  return (
    <div className="bg-mainWhite-50 md:py-gaps-md py-gaps px-sides sm:px-sides-sm items-center flex flex-col lg:rounded-b-[160px] gap-28 mobile:gap-14">
      <div className="items-center flex flex-col gap-10 max-w-[39rem]">
        ...
        <ul className=" bg-mainWhite-0 border-t-8 border-mainOrange-700 rounded-base flex flex-col gap-4 md:gap-10 p-sides">
          {items.map((item, i) => (
            <li key={i} className="flex gap-4 items-center">
              <ApproveSvg />
              <p className=" text-16 md:text-18 leading-30 text-mainGray-600">{item}</p>
            </li>
          ))}
        </ul>
        <LinkCmp
